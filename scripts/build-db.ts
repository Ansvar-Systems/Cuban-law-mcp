#!/usr/bin/env tsx
/**
 * Database builder for Cuba Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Cuba Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Cuban statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'CU');
    insertMeta.run('source', 'gacetaoficial.gob.cu');
    insertMeta.run('licence', 'Government Official Gazette');
  });
  writeMeta();
